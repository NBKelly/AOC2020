...
public class Advent06 extends ConceptHelper {
    //BE AWARE OF:
	...
	int partOne_total = 0;
        while(hasNextLine()) {	    
	    TreeSet<Character> partTwo = new TreeSet<>();
	    TreeSet<Character> partOne = new TreeSet<>();

	    boolean first = true;
	    String line = "";
	    //while we have new lines. When we get an empty line, we break
	    while(hasNextLine() && !(line = nextLine()).equals("empty")) {
		TreeSet<Character> charset = new TreeSet<>();
		for(int i = 0; i < line.length(); i++)
		    charset.add(line.charAt(i));
		if(first) {
		    partTwo.addAll(charset);
		    first = false;
		} else {
		    
		    partTwo.retainAll(charset);	//NOTE: retainAll is intersection
		}

		partOne.addAll(charset);
	    }

	    partTwo_total += partTwo.size();
	    partOne_total += partOne.size();
	}

	DEBUGF("PART ONE: ");
	println(partOne_total);

	DEBUGF("PART TWO: ");
	println(partTwo_total);
